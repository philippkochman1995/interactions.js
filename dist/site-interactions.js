...
var G = {
	...
	revealDuration: .92,
	ease: "power4.inOut"
}, ht = "page-transition-overlay", gt = "[data-page-transition-overlay], .page-transition-overlay", K = "site-page-transition", q = "pending", _t = "is-page-transition-pending", vt = [
...
function Y(e) {
	document.documentElement.classList.toggle(_t, e);
}
function bt() {
	try {
		Y(window.sessionStorage.getItem(K) === q);
	} catch (e) {
		Y(!1);
	}
}
...
function Tt(e, t) {
	return !J && !e.defaultPrevented && !St(e) && !Ct(t);
}
function Et() {
	try {
		window.sessionStorage.setItem(K, q), Y(!0);
	} catch (e) {}
}
function Dt() {
	try {
		let e = window.sessionStorage.getItem(K) === q;
		return window.sessionStorage.removeItem(K), Y(!1), e;
	} catch (e) {
		return Y(!1), !1;
	}
}
...
function jt() {
	if (yt) return;
	if (!document.body) {
		document.addEventListener("DOMContentLoaded", jt, { once: !0 });
		return;
	}
	yt = !0;
	let e = xt();
...
var Mt = "[data-site-menu]", Nt = "[data-site-menu-panel]", Pt = "[data-site-menu-toggle]", Ft = "[data-site-menu-toggle-label]", It = "[data-site-menu-link]", Lt = "[data-site-menu-indicator]", Rt = "is-active", X = "is-open", zt = "is-ready", Bt = "data-site-menu-open-label", Vt = "data-site-menu-closed-label", Z = "data-site-menu-current-key", Ht = "data-site-menu-label", Ut = "data-site-menu-key", Q = "data-site-menu-original-tabindex", Wt = "CLOSE", Gt = "MENU", Kt = [], qt = !1;
function Jt(e) {
...
function Yt(e) {
	...
	try {
		return Jt(new URL(e.href, window.location.href).pathname);
	} catch (e) {
		return "";
	}
}
...
function Zt(e, t) {
	var n, r;
	if (e.classList.contains("w--current") || e.getAttribute("aria-current") === "page" || Xt(e, m(t, Z) || ((n = document.documentElement.getAttribute(Z)) == null ? void 0 : n.trim()) || ((r = document.body.getAttribute(Z)) == null ? void 0 : r.trim()) || "")) return !0;
	let i = Yt(e);
	return i ? i === Jt(window.location.pathname) : !1;
}
function Qt(e) {
	var t, n;
	return m(e, Ht) || ((t = (n = e.textContent) == null ? void 0 : n.replace(/\s+/g, " ").trim()) == null ? "" : t);
}
function $t(e) {
	var t;
	let n = (t = e.links.find((e) => e.classList.contains(Rt) || e.classList.contains("w--current"))) == null ? e.links.find((t) => Zt(t, e.root)) : t;
	return n ? Qt(n) : "";
}
function en(e, t = !0) {
	var n;
	let r = m(e.root, Bt) || Wt, a = m(e.root, Vt) || Gt, o = $t(e), s = e.isOpen ? r : e.isHovered ? a : o || a, c = (n = e.toggleLabel) == null ? e.toggle : n;
	if (c.textContent !== s) {
...
function tn(e, t) {
	e.links.forEach((e) => {
		...
		!e.hasAttribute(Q) && e.hasAttribute("tabindex") && e.setAttribute(Q, String(e.tabIndex)), e.setAttribute("tabindex", "-1");
	});
}
function nn(e, t, n = !0) {
	e.isOpen = t, e.root.classList.toggle(X, t), e.toggle.setAttribute("aria-expanded", String(t)), e.panel.setAttribute("aria-hidden", String(!t)), tn(e, t), en(e, n);
}
function rn(e, t, n) {
...
function cn(e) {
	...
	let i = {
		...
		links: l(It, e),
		isOpen: e.classList.contains(X),
		isHovered: !1,
		cleanup: []
	};
	r.type || (r.type = "button"), n.id || (n.id = `site-menu-panel-${Kt.length + 1}`), r.setAttribute("aria-controls", n.id), sn(i), nn(i, i.isOpen, !1), e.classList.add(zt);
	let a = (e) => {
		e.preventDefault(), on(i);
	}, o = (t) => {
		...
		let t = e.target;
		!(t instanceof Element) || !t.closest(It) || $(i);
	}, u = () => {
		i.isHovered = !0, en(i);
	}, d = () => {
		i.isHovered = !1, en(i);
...
function ln(e = document) {
	if (qt && e === document) return () => void 0;
	e === document && (qt = !0);
	let t = l(Mt, e).map(cn).filter((e) => !!e);
	return Kt.push(...t), () => {
		t.forEach((e) => {
			var t, n;
			e.cleanup.forEach((e) => e()), e.root.classList.remove(zt, X), v.killTweensOf(e.panel), v.killTweensOf((t = e.toggleLabel) == null ? e.toggle : t), v.set(e.panel, { clearProps: "height" }), v.set((n = e.toggleLabel) == null ? e.toggle : n, { clearProps: "opacity" }), e.panel.removeAttribute("aria-hidden"), e.toggle.removeAttribute("aria-expanded"), tn(e, !0);
		});
	};
}
//#endregion
...
function dn() {
	...
	un = !0;
	let e = te();
	mt({ i18n: e }), Pe({ i18n: e }), ln(), n(), window.SiteInteractions = {
		openModal: U,
		openContentModal: H,
		closeModal: W,
		openLightbox: M,
