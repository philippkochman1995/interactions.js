...
type WorksSortMode = 'curated' | 'random' | 'alphabetical' | 'year';
...
function filterItems(items: WorkItem[], selectedCategories: Set<string>): WorkItem[] {
  if (selectedCategories.size === 0) {
    return items;
  }

  return items.filter((item) => item.categories.some((category) => selectedCategories.has(category)));
}

function getCategoryCounts(items: WorkItem[]): CategoryCount[] {
...
function createFilterInterface(
  ...
  wrapper.className = 'cms-works-filter u-section u-container';
  ...
  wrapper.append(overlay, topbar, panel);
  ...
  function sync(): void {
    ...
    categoryButtons.forEach((button, category) => {
      ...
      if (buttonCount) {
        buttonCount.textContent = active ? 'x' : buttonCount.textContent?.replace('x', '') || '';
        if (!active) {
  ...
  document.addEventListener('pointerdown', (event) => {
    if (!state.open || wrapper.contains(event.target as Node)) {
      return;
    }
...
function renderWorks(root: HTMLElement, source: HTMLElement): void {
  ...
  Promise.all(items.map(async (item) => [item.id, await measureImage(item.thumbnail)] as const)).then((entries) => {
    ...
    const controls = createFilterInterface(root, items, state, () => {
